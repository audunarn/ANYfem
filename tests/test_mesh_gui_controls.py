...
def test_explicit_same_id_reopen_discards_draft_and_rehydrates_saved_controls():
...
def test_build_trace_sequence_and_first_hydration_without_tk(monkeypatch):
    ...
    assert panel._mesh_controls_value() == alpha_controls()
    assert panel._metric_combo.options["state"] == "readonly"
    assert panel._controls_dirty is False
...
def test_persistence_snapshot_and_refresh_keep_exact_controls():
...
def test_legacy_ignores_disabled_budget_draft_and_retains_saved_budget():
    project = Project("legacy")
    store(project, alpha_controls())
    panel = panel_for(project)
    panel._native_max_insertions.set("")
    assert MeshPanel._mesh_controls_value(panel).max_insertions == 321
...
def test_mapped_ignores_hidden_invalid_fields_but_keeps_saved_native_options():
    ...
    assert controls == replace(alpha_controls(), certification_mode="interactive")
...
def test_async_submission_records_controls_without_starting_worker():
...
def test_worker_propagates_controls_to_project_without_meshing(monkeypatch):
    captured = []
    ...
    try:
        ...
        assert captured[0]["mesh_controls"] == alpha_controls()
...
def test_project_passes_public_owner_options_without_meshing(monkeypatch, strategy):
    ...
    with pytest.raises(RuntimeError, match="test boundary"):
        project.generate_mesh(0.25, strategy=strategy, mesh_controls=alpha_controls())
    assert captured[0]["certification_mode"] == "strict"
    assert captured[0]["recombine"] is False
    if strategy == "mapped":
        assert "native_options" not in captured[0]
    else:
        assert captured[0]["native_options"].to_dict() == alpha_controls().native_options().to_dict()
